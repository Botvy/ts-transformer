    ...
    return (sourceFile) => {
      const visitor = (node: ts.Node): ts.Node => {
        if (ts.isIdentifier(node)) {
          ...
          if (
            foundSymbols.some((foundSymbol) => foundSymbol === relatedSymbol)
          ) {
            const foundIndex = foundSymbols.indexOf(relatedSymbol!);
            console.log(
              `Found existing symbol at position = ${foundIndex} and name = "${
                relatedSymbol?.name ?? ""
              }"`
            );
          } else {
            ...
            console.log(
              `Found new symbol with name ${
                ts.SyntaxKind[relatedSymbol.declarations[0].kind]
              } = "${relatedSymbol?.name ?? ""}". Added at positon = ${
